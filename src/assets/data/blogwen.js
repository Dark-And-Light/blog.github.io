let blog_wen = [
    {
        ...
        first:'在面试中被问到有没有了解到路由守卫，当时回答的是没有了解过，但当时有模糊的概念，因为在做小程序时遇到过类似的情况，在看别人的小程序时，遇到过只有登录后才展示的页面，经过我后来的了解，路由守卫分为全局前置守卫、路由独享的守卫、全局后置钩子、vue2.5新增全局解析守卫',
        content:"<p>在面试中被问到有没有了解到路由守卫，当时回答的是没有了解过，但当时有模糊的概念，因为在做小程序时遇到过类似的情况，在看别人的小程序时，遇到过只有登录后才展示的页面，经过我后来的了解，路由守卫分为全局前置守卫、路由独享的守卫、全局后置钩子、vue2.5新增全局解析守卫</p><p>个人理解是路由守卫就是在页面进入页面或者跳转页面时，对页面显示进行控制，to：即将进入的目标页面，from:当前正要离开的页面，next()没有理解，应该是只有执行next()才能展示页面内容，next中也能传送参数如地址或者布尔值，以及error</p>"
    },
    {
        id:6,
        label:"Vue",
        ...
        content:'<p>在设计过程中遇到一个不是很理解的地方，也能别人设计的博客使用的框架不一样，他人文章页面跳转时会在当前页面的地址栏中出现blog/xxx.html，一开始我的想法是我可能没有学习到的vue新知识，后来百度查也没查出来，然后其实我并不需要完全一样，地址栏是具有欺骗性的，我也可以通过路由地址的形式不过不是以html文件的形式在app的<router-view/>中呈现我的子页面。我说的不是很清楚，认真观察地址栏的跳转就明白我说了</p>'
